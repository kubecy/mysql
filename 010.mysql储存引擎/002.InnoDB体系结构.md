...
1. 概念：
   - 表是数据库中的基本数据结构，用于存储相关数据。每个表由行和列组成，行表示记录，列表示属性或字段。

### 2.2.Indexes（索引）

1. 概念：
   - 用于加速数据检索的一种数据结构，它通过特定的方式组织数据，允许快速查找、插入和删除操作。索引可以显著提高数据库查询性能
...
1. 概念：
   ...
   - 当 InnoDB 需要将数据页写入磁盘时，它首先将数据页写入双写缓冲区。然后，再将双写缓冲区中的数据页写入到实际的表空间文件中。这一过程确保了如果在写入过程中发生崩溃或故障，仍然可以从双写缓冲区中恢复数据。

2. DWB文件主要作用是：在数据库服务存储时，数据页写了一半，数据库Innodb可以找到一个好的数据页副本从Doublewrite Buffer文件中，主要是避免数据信息出现损坏；MySQL数据库最小IO存储单元是page(16kB)，OS系统中最小的IO存储单元是block(4kB)，OS也可以称为存储子系统；

   - 会出现一个问题：数据库系统与操作系统的存储关系问题，在数据库中写入一个数据页时，在文件系统层面可能只是写入了2个block；

   ![image-20241019190233694](./000.picture/image-20241019190233694.png)

>问题解决思路

![image-20241019190303637](./000.picture/image-20241019190303637.png)

在数据库8.0.19之前，默认在ibdataN文件中进行存储，在数据库8.0.20以后，可以进行独立文件存储；

~~~shell
root@mysql-db111 /app/mysql/data # ll *#ib_16384_*  
-rw-r----- 1 mysql mysql  196608 Oct 20 00:52 '#ib_16384_0.dblwr'
-rw-r----- 1 mysql mysql 8585216 Sep 26 11:29 '#ib_16384_1.dblwr'
~~~
...
1. 概念
   ...
   - 要用于记录内存数据页的变化（记录在内存中对数据页的操作信息），都会以日志文件方式记录；
...
~~~sql
root@mysql-db111 /app/mysql/data # ll ib_log*
-rw-r----- 1 mysql mysql 2147483648 Oct 20 00:52 ib_logfile0
-rw-r----- 1 mysql mysql 2147483648 Oct 20 00:20 ib_logfile1
-rw-r----- 1 mysql mysql 2147483648 Oct 20 00:20 ib_logfile2


-- 1、redo事务日志配置信息查看，在实际生产环境中，建议大小为512M~4G，应用组数为2~4组（写入数据过程轮询写入）
...
1. **概念**：
   ...
   - 撤销日志是 InnoDB 存储引擎用于记录事务修改之前的状态，以便在需要时进行回滚操作，确保数据的一致性和完整性。
   - 当事务对数据进行修改时，InnoDB 会在撤销日志中记录修改前的数据快照。这些记录包含了如何将数据恢复到修改前的状态。
   - 在事务回滚时，撤销日志会被用于逆向应用修改，从而将数据恢复到之前的状态。

2. **调整参数**：通过修改 MySQL 配置文件（如 `my.cnf`）中的参数，控制撤销日志的行为
